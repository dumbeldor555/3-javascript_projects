...
const generate = document.querySelector('.genrater');

generate.addEventListener('click', showPara);

function showPara() {
  if(counter.value <= 0) {

    paras.innerHTML = '<p> plese enter a choose a number greater then "0" </P>';
  } else {
  ...
  for(let i = 0; i < counter.value; i++) {
    ...
    const mytext = document.createTextNode(text[i]);
